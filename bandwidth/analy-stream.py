...
for thread_num in thread_nums:
    ...
    # 実行
    process = subprocess.run("numactl -l " + elf, shell=True, capture_output=True, text=True, env=env)
    output = process.stdout
    
    # 結果を解析して保存
    for line in output.splitlines():
        if "Copy:" in line:
            print(thread_num,line.split()[1])
            copy_result = float(line.split()[1])/2**10
        elif "Scale:" in line:
            scale_result = float(line.split()[1])/2**10
        elif "Add:" in line:
            add_result = float(line.split()[1])/2**10
        elif "Triad:" in line:
            triad_result = float(line.split()[1])/2**10
...
def plot_results(results, operation):
    ...
    output_dir = "stream_results_core"
